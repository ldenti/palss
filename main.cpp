...
// /* Sweep line clustering of specific strings based on their subgraphs -
// assuming
...
//                       c.second[last].s + c.second[last].l -
//                       c.second[first].s, c.second[first].a, c.second[last].b,
...
int main(int argc, char *argv[]) {
  ...
  //         "
  //         "%d/%d strings skipped)\n",
  //         __func__, realtime() - rt, lowsc_n, lowsc_am_n, lowsc_n +
  //         lowsc_am_n, skipped, total);
  // rt = realtime();
  // // ---

  // // Specific strings cleaning (make sure they start/end with "correct"
  // kmers) vector<vector<sfs_t *>> pspecifics(qnames.size()); for (auto &c :
  // Cs) {
  ...
  //         s->good = 0;
  //       } else {
  //         s->a.seq = ckmer_d; // FIXME: change also the vertex or don't
  //         change
  ...
  //         s->good = 0;
  //       } else {
  //         s->b.seq = ckmer_d; // FIXME: change also the vertex or don't
  //         change
  //                             // anything and mark the anchors as not updated
  //         s->l += p + k - (s->s + s->l);
  //       }
  ...
  // gap_open2+gap_len*gap_ext2} abpoa_post_set_para(abpt);
