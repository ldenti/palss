...
rule run:
    input:
        expand(pjoin(WD, "{n}", "k{k}", "missed_regions.fa"), n=Ns, k=Ks),
...
rule rb3_index:
    input:
        rfa=FA,
        hfa=rules.vg_extracthaplotypes.output.fa,
    output:
        fmd=pjoin(WD, "{n}", "pangenome.fmd"),
    log:
...
rule bed2fa:
    ...
    output:
        fa=pjoin(WD, "{n}", "k{k}", "missed_regions.fa"),
    conda:
        "./envs/bedtools.yml"
    shell:
        """
