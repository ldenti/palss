...
rule palss_search:
    input:
        ...
        reads=pjoin(WD, "cov{cov}", sample + "-reads.ec.fa"),
    output:
        sfs=pjoin(WD, "n{n}", "palss-{graph}", "cov{cov}", "specific_strings.d{d}.txt"),
    threads: workflow.cores
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "cov{cov}", "search.d{d}.time"),
    shell:
        """
...
rule palss_align:
    input:
        ...
        sfs=pjoin(WD, "n{n}", "palss-{graph}", "cov{cov}", "specific_strings.d{d}.txt"),
    output:
        gaf=pjoin(WD, "n{n}", "palss-{graph}", "cov{cov}", "consensus.d{d}.gaf"),
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "cov{cov}", "align.d{d}.time"),
    threads: workflow.cores
    shell:
        """
...
#     shell:
#         """
#         # /usr/bin/time -vo {log.time} sh -c "vg augment --min-coverage 1 --gaf {input.gfa} {input.gaf} > {output.gfa}.redundant ; python3 ../scripts/remove_redundant_bubbles.py {output.gfa}.redundant | vg mod --unchop - > {output.gfa}"
#         /usr/bin/time -vo {log.time} sh -c "vg augment --min-coverage 1 --gaf {input.gfa} {input.gaf} > {output.gfa}"
#         """
