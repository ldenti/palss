...
rule palss_sketch:
    ...
    output:
        skt=pjoin(WD, "n{n}", "pangenome-{graph}.d{d}.skt"),
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "sketch.d{d}.time"),
...
rule palss_fmd:
    input:
        gbz=pjoin(WD, "n{n}", "pangenome-{graph}.gbz"),
    output:
        fmd=pjoin(WD, "n{n}", "pangenome-{graph}.fmd"),
    threads: workflow.cores
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "fmd.time"),
    shell:
        """
...
rule palss_search:
    ...
    output:
        sfs=pjoin(WD, "n{n}", "palss-{graph}", "specific_strings.d{d}.txt"),
    threads: workflow.cores
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "search.d{d}.time"),
    shell:
        """
...
rule palss_sam:
    input:
        ...
        sfs=pjoin(WD, "n{n}", "palss-{graph}", "specific_strings.d{d}.txt"),
    output:
        bam=pjoin(WD, "n{n}", "palss-{graph}", "specific_strings.d{d}.bam"),
    shell:
        """
...
rule palss_align:
    input:
        ...
        sfs=pjoin(WD, "n{n}", "palss-{graph}", "specific_strings.d{d}.txt"),
    output:
        gaf=pjoin(WD, "n{n}", "palss-{graph}", "consensus.d{d}.gaf"),
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "align.d{d}.time"),
    threads: workflow.cores
    shell:
        """
...
rule palss_augment:
    ...
    output:
        gfa=pjoin(WD, "n{n}", "palss-{graph}", "pangenome-augmented.d{d}.w{w}.gfa"),
        gaf=pjoin(WD, "n{n}", "palss-{graph}", "anchored-consensus.d{d}.w{w}.gaf"),
    params:
        wd=pjoin(WD, "n{n}", "palss-{graph}", "augment.d{d}.w{w}.wd"),
        log=pjoin(WD, "n{n}", "palss-{graph}", "augment.d{d}.w{w}.log"),
    conda:
        "../envs/graphaligner.yaml"
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "augment.d{d}.w{w}.time"),
...
rule palss_refine_1:
    ...
    params:
        wd=pjoin(WD, "n{n}", "palss-{graph}", "augment-refine.d{d}.w{w}.wd"),
        log=pjoin(WD, "n{n}", "palss-{graph}", "augment-refine.d{d}.w{w}.log"),
    conda:
        "../envs/refine.yaml"
    threads: workflow.cores
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "refine_1.d{d}.w{w}.time"),
    shell:
        """
        mkdir -p {params.wd}
...
rule palss_refine_2:
    ...
    threads: workflow.cores
    log:
        time=pjoin(WD, "times", "n{n}", "palss-{graph}", "refine_2.d{d}.w{w}.time"),
    shell:
        """
...
rule palss_refine_3:
    ...
    output:
        gaf=pjoin(
            ...
            "n{n}",
            "palss-{graph}",
            "unanchored_consensus.d{d}.w{w}.c{c}.m{m}.gaf",
        ),
    threads: max(1, workflow.cores / 4)
    conda:
        "../envs/biopython.yaml"
    log:
        time=pjoin(
            WD, "times", "n{n}", "palss-{graph}", "refine_3.d{d}.w{w}.c{c}.m{m}.time"
        ),
        log=pjoin(
            WD,
            "n{n}",
...
rule palss_refine_4:
    ...
    log:
        time=pjoin(
            WD, "times", "n{n}", "palss-{graph}", "refine_4.d{d}.w{w}.c{c}.m{m}.time"
        ),
    shell:
        """
        /usr/bin/time -vo {log.time} sh -c "vg augment --min-coverage 1 --gaf {input.gfa} {input.gaf} | vg mod --unchop - > {output.gfa}.redundant ; python3 ../scripts/remove_redundant_bubbles.py {output.gfa}.redundant > {output.gfa}"
        """
...
rule gaf2fa:
    ...
    output:
        fa=pjoin(WD, "n{n}", "palss-{graph}", "anchored-consensus.d{d}.w{w}.fa"),
    shell:
        """
