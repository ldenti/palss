...
def main():
    ...
    if not args.s:
        for col, n in enumerate(Ns):
            ...
            lookup = (
                pdf_v[(pdf_v["n"] == n)]
                .set_index([x_name, hue_name])["P_vertices"]
                .to_dict()
            )
            ...
            lookup = (
                pdf_e[(pdf_e["n"] == n)]
                .set_index([x_name, hue_name])["P_edges"]
                .to_dict()
            )
            lookup = {k: round(v * 100, 1) for k, v in lookup.items()}
            ...
            lookup = (
                pdf_v[(pdf_v["n"] == n)]
                .set_index([x_name, hue_name])["P_bases"]
                .to_dict()
            )
            lookup = {k: round(v * 100, 1) for k, v in lookup.items()}
            ...
            # annotation
            lookup = rdf[(rdf["n"] == n)].set_index([x_name, hue_name])["R"].to_dict()
            lookup = {k: round(v * 100, 1) for k, v in lookup.items()}
    ...
    axes[0][0].set_ylabel("Correctness (vertices)")
    axes[1][0].set_ylabel("Correctness (edges)")
    axes[2][0].set_ylabel("Correctness (bases)")
    axes[2][0].ticklabel_format(axis="y", style="plain", useOffset=False)
    axes[3][0].set_ylabel("Completeness")
    for i in range(3):
        # axes[3][i].set_xlabel(f"Graph (n={Ns[i]})")
        axes[3][i].set_xlabel(f"Coverage")
        axes[0][i].set_title(f"n={Ns[i]}", pad=21)
