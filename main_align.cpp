...
int main_align(int argc, char *argv[]) {
  ...
  for (size_t cidx = 0; cidx < clusters.size(); ++cidx) {
    ...
    // for (path_t &p : paths) {
    //   std::cerr << p.id << " " << graph.get_path_contig(p.id >> 1) << " " <<
    //   p.vertices.size()
    //             << std::endl;
    //   for (const auto &v : p.vertices)
    //     std::cerr << graph.get_gfa_name(v >> 1) << " ";
    //   std::cerr << std::endl;
    ...
    if (first_kmer.empty() && last_kmer.empty()) {
      ...
      ++noanchors_clusters;
      fprintf(stderr,
              "Skipping cluster since could not find anchors on any path, "
              "kmers: %d\n",
              hit);
      continue;
    }
