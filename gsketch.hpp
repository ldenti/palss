...
inline uint64_t encode(int v, int off) {
  uint64_t e = ((uint64_t)v << 16) | (off & 0xFFFF);
  return e;
}

inline int64_t decode_v(uint64_t e) { return e >> 16; }

inline int16_t decode_off(uint64_t e) { return e & 0xFFFF; }
...
public:
  ...
  map<uint64_t, uint64_t> sketch; // sketch : (48[vertex, 16[offset)
